...
class FullTree extends React.Component {
  ...
  render() {
    ...
    if (tree.obj && tree.obj.children && tree.obj.children.length == 0 && this.props.searchTerm) {
      return (<div className="m-tree empty-search"> no search results for {this.props.searchTerm}</div>)
    } else {
  ...
  dragEnd() {
    ...
    if (index) {
      ...
      let oldParent = indexBeforeDrag.parent ? tree.getIndex(indexBeforeDrag.parent).node : null
  ...
  change(tree, node, oldParent, parent, index) {
